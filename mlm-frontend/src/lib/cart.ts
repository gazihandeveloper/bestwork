...
// Ürünü PERAKENDE (indirimsiz/katalog fiyatlı) olarak sepete ekler — paket yükseltme alımı.
export function addRetailToCart(product: Product, quantity: number): CartItem[] {
  const items = loadCart();
  const qty = Math.max(1, Math.min(quantity, product.stock));
  const existing = items.find((c) => c.product.id === product.id);
  if (existing) {
    existing.quantity = Math.min(existing.quantity + qty, product.stock);
    existing.retail = true;
  } else {
    items.push({ product, quantity: qty, retail: true });
  }
  saveCart(items);
  return items;
}
...
export function removeFromCart(productId: number): CartItem[] {
  const items = loadCart().filter((c) => c.product.id !== productId);
  saveCart(items);
  return items;
}

// Ürünün sepet adedini tam olarak ayarlar (0 = sil, >0 = güncelle).
// Mevcut retail (paket yükseltme) bayrağı korunur; yeni ürün normal (indirimli) eklenir.
export function setCartQuantity(product: Product, quantity: number): CartItem[] {
  const items = loadCart();
  const qty = Math.max(0, Math.min(quantity, product.stock));
  const idx = items.findIndex((c) => c.product.id === product.id);
  if (idx >= 0) {
