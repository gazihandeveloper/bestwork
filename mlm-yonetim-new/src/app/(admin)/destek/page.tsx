  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
      <div className="mb-4 flex items-center justify-between">
        <div>
          <h2 className="text-lg font-bold text-gray-800 dark:text-white/90">Destek Talepleri</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400">Talebe tıklayın — yazışma açılır, yanıt yazabilirsiniz.</p>
        </div>
        <span className="rounded-full bg-gray-100 dark:bg-white/10 px-3 py-1 text-sm font-bold text-gray-600 dark:bg-white/10 dark:text-gray-300">{tickets.length} talep</span>
      </div>

      {error && <div className="mb-3 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700 dark:border-red-500/30 dark:bg-red-500/10 dark:text-red-400">{error}</div>}

      {loading ? (
        <div className="py-10 text-center text-gray-400 dark:text-gray-500">Yükleniyor...</div>
      ) : tickets.length === 0 ? (
        <div className="py-10 text-center text-gray-400 dark:text-gray-500">Henüz destek talebi yok.</div>
      ) : (
        <div className="overflow-x-auto">
          <table className="w-full min-w-[840px] text-sm">
            ...
            <tbody>
              {tickets.map((t) => {
                const st = STATUS[t.status] ?? { label: t.status, cls: "bg-gray-100 text-gray-600 dark:bg-white/10 dark:text-gray-300" };
                const openRow = t.status === "open" || t.status === "new";
                return (
                  <tr key={t.id} className="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50">
                    <td className="px-3 py-2">
                      <button type="button" onClick={() => open(t)} className="cursor-pointer font-mono font-bold text-brand-600 hover:underline">
                        {code(t)}
                      </button>
                    </td>
                    <td className="px-3 py-2 font-semibold text-gray-800 dark:text-white/90">{t.name} {t.surname}</td>
          ...
          <div className="flex max-h-[90vh] w-full max-w-lg flex-col rounded-2xl bg-white shadow-xl dark:bg-gray-900" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between border-b border-gray-100 dark:border-gray-800 px-5 py-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <button type="button" onClick={() => setSelected(null)} className="cursor-pointer rounded-lg p-1 text-gray-400 hover:bg-gray-100 dark:hover:bg-white/5">
                  ✕
                </button>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4">
              <p className="text-sm whitespace-pre-wrap leading-relaxed text-gray-700 dark:text-gray-300">{selected.message}</p>
            </div>
