  ...
  return (
    <div className="relative flex">
      ...
      {selectPosition === "start" && (
        <div className="absolute">
          ...
          >
            {countries.map((country) => (
              <option
                key={country.code}
                value={country.code}
                className="text-gray-700 dark:bg-[#1E293B] dark:text-gray-400"
              >
                {country.code}
              </option>
            ))}
          ...
          <div className="absolute inset-y-0 flex items-center text-gray-700 pointer-events-none bg-none right-3 dark:text-gray-400">
            <ChevronDownIcon className="stroke-current" size={20} />
          </div>
        </div>
      )}
      ...
      <input
        ...
        } rounded-lg border border-gray-300 bg-transparent py-3 px-4 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-[#1E293B] dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800`}
      />
