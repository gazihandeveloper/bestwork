...
func main() {
	...
	earningPlanService := services.NewEarningPlanService(database.GetDB())
	...
	earningPlanHandler := handlers.NewEarningPlanHandler(earningPlanService)
	...
	adminHandler := handlers.NewAdminHandler(walletService, chipService, monthlyCloseService, pendingPoolService, paymentNotificationService, userService, orderService, auditService, adminStatsService, treeAdminService, jobService)
	...
	api := router.Group("/api")
	{
		...
		// Eshop storefront (bestwork2) uyumlu katalog endpoint'leri.
		eshopHandler := handlers.NewEshopHandler(productService, categoryService)
		api.GET("/eshop/products", eshopHandler.ListProducts)
		api.GET("/eshop/products/slug/:slug", eshopHandler.GetProductBySlug)
		api.GET("/eshop/categories", eshopHandler.ListCategories)

		// Eshop storefront uyumlu auth endpoint'leri (JWT tabanlı sözleşme).
		eshopAuthHandler := handlers.NewEshopAuthHandler(userService)
		api.POST("/eshop/auth/login", middleware.RateLimit(database.GetRedis(), "eshop-login", 5, time.Minute), eshopAuthHandler.Login)
		api.POST("/eshop/auth/register", middleware.RateLimit(database.GetRedis(), "eshop-register", 10, time.Hour), eshopAuthHandler.Register)
		api.POST("/eshop/auth/refresh", middleware.RateLimit(database.GetRedis(), "eshop-refresh", 30, time.Minute), eshopAuthHandler.Refresh)
		eshopMe := api.Group("/eshop")
		eshopMe.Use(middleware.AuthRequired(userService))
		eshopMe.GET("/users/me", eshopAuthHandler.Me)

		// Eshop storefront uyumlu hesabım endpoint'leri (sipariş + cüzdan + MLM dashboard).
		eshopAccountHandler := handlers.NewEshopAccountHandler(orderService, walletService, productService, dashboardService, userService, packageService, pendingPoolService, ticketService)
		eshopAcct := api.Group("/eshop")
		eshopAcct.Use(middleware.AuthRequired(userService))
		eshopAcct.GET("/orders", eshopAccountHandler.ListOrders)
		eshopAcct.GET("/orders/:id", eshopAccountHandler.GetOrder)
		eshopAcct.GET("/wallet", eshopAccountHandler.GetWallet)
		eshopAcct.GET("/wallet/transactions", eshopAccountHandler.ListWalletTransactions)
		eshopAcct.GET("/dashboard", eshopAccountHandler.GetDashboard)
		eshopAcct.GET("/me", eshopAccountHandler.GetMeSummary)
		eshopAcct.GET("/ranks", eshopAccountHandler.ListRanks)
		eshopAcct.GET("/packages", eshopAccountHandler.ListPackages)
		eshopAcct.GET("/sponsored", eshopAccountHandler.ListSponsored)
		eshopAcct.GET("/pending-pool", eshopAccountHandler.ListPendingPool)
		eshopAcct.GET("/tickets", eshopAccountHandler.ListTickets)
		eshopAcct.GET("/tickets/:id", eshopAccountHandler.GetTicket)
		eshopAcct.POST("/tickets/:id/reply", eshopAccountHandler.ReplyTicket)
		eshopAcct.GET("/profile", eshopAccountHandler.GetProfile)
		eshopAcct.PUT("/profile-image", eshopAccountHandler.SetProfileImage)
		api.GET("/packages", packageHandler.List)
		api.GET("/ranks", rankHandler.List)
		api.GET("/announcements", announcementHandler.ListActive)
		api.GET("/hero-slides", heroSlideHandler.List)
		api.GET("/benefits", benefitHandler.List)
		api.GET("/settings", settingHandler.Get)
		// Herkese açık oturum kontrolü (401 üretmez; frontend sessiz oturum kontrolü için kullanır).
		api.GET("/user/me/status", userHandler.Status)
		...
		// Panel sözleşmesiyle uyumlu alias: /admin/products
		adminProductsAlias := api.Group("/admin/products")
		adminProductsAlias.Use(adminOnly()...)
		adminProductsAlias.POST("", productHandler.Create)
		adminProductsAlias.PUT("/:id", productHandler.Update)
		adminProductsAlias.DELETE("/:id", productHandler.Delete)

		adminCategories := api.Group("/admin/categories")
		adminCategories.Use(adminOnly()...)
		...
		adminEarningPlans := api.Group("/admin/earning-plans")
		adminEarningPlans.Use(adminOnly()...)
		adminEarningPlans.GET("", earningPlanHandler.List)
		adminEarningPlans.POST("", earningPlanHandler.Create)
		adminEarningPlans.GET("/:id", earningPlanHandler.Get)
		adminEarningPlans.PUT("/:id", earningPlanHandler.Update)
		adminEarningPlans.DELETE("/:id", earningPlanHandler.Delete)

		adminAnnouncements := api.Group("/admin/announcements")
		adminAnnouncements.Use(adminOnly()...)
		adminAnnouncements.GET("", announcementHandler.ListAll)
		...
		// Seviye (rütbe) yönetimi — admin CRUD
		adminRanks := api.Group("/admin/ranks")
		adminRanks.Use(adminOnly()...)
		adminRanks.GET("", rankHandler.ListAll)
		adminRanks.POST("", rankHandler.Create)
		adminRanks.PUT("/:id", rankHandler.Update)
		adminRanks.DELETE("/:id", rankHandler.Delete)
		...
		api.POST("/admin/wallet/transfer", append(superAdminOnly(), adminHandler.TransferWallet)...)
		api.GET("/admin/wallet/transfers", append(adminOnly(), adminHandler.ListTransfers)...)
		...
		api.GET("/admin/financial-summary", append(adminOnly(), adminHandler.FinancialSummary)...)
		api.GET("/admin/top-earners", append(adminOnly(), adminHandler.TopEarners)...)
		api.GET("/admin/revenue", append(adminOnly(), adminHandler.Revenue)...)
