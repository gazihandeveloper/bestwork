...
const productColumns = `id, name, price, pv, cv, stock, description, image_path, category, sku, created_at`

// scanProduct tek satırı models.Product'a dönüştürür.
func scanProduct(row pgx.Row) (*models.Product, error) {
	var p models.Product
	if err := row.Scan(&p.ID, &p.Name, &p.Price, &p.PV, &p.CV, &p.Stock, &p.Description, &p.ImagePath, &p.Category, &p.SKU, &p.CreatedAt); err != nil {
...
func (s *ProductService) CreateProduct(ctx context.Context, name, description, imagePath, category, sku string, price float64, pv, cv int64, stock int) (*models.Product, error) {
	...
	err := s.db.QueryRow(ctx,
		`INSERT INTO products (name, price, pv, cv, stock, description, image_path, category, sku)
		 VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING id, created_at`,
		p.Name, p.Price, p.PV, p.CV, p.Stock, p.Description, p.ImagePath, p.Category, p.SKU).Scan(&p.ID, &p.CreatedAt)
	if err != nil {
...
func (s *ProductService) GetProductByID(ctx context.Context, id int64) (*models.Product, error) {
	return scanProduct(s.db.QueryRow(ctx, `SELECT `+productColumns+` FROM products WHERE id = $1`, id))
}
...
func (s *ProductService) ListProducts(ctx context.Context, q string) ([]models.Product, error) {
	...
	if q = strings.TrimSpace(q); q != "" {
		...
		query += ` WHERE unaccent(lower(name)) LIKE unaccent(lower($1)) OR unaccent(lower(sku)) LIKE unaccent(lower($1))`
		args = append(args, "%"+q+"%")
	}
	query += ` ORDER BY id`
	...
	for rows.Next() {
		var p models.Product
		if err := rows.Scan(&p.ID, &p.Name, &p.Price, &p.PV, &p.CV, &p.Stock, &p.Description, &p.ImagePath, &p.Category, &p.SKU, &p.CreatedAt); err != nil {
			return nil, fmt.Errorf("ürün okunamadı: %w", err)
		}
...
// UpdateProduct ürünün tüm değişebilir alanlarını günceller.
func (s *ProductService) UpdateProduct(ctx context.Context, p *models.Product) error {
	tag, err := s.db.Exec(ctx,
		`UPDATE products SET name = $1, price = $2, pv = $3, cv = $4, stock = $5, description = $6, image_path = $7, category = $8, sku = $9 WHERE id = $10`,
		p.Name, p.Price, p.PV, p.CV, p.Stock, p.Description, p.ImagePath, p.Category, p.SKU, p.ID)
	if err != nil {
...
func (s *ProductService) ListPopular(ctx context.Context, limit, days int) ([]PopularProduct, error) {
	...
	rows, err := s.db.Query(ctx, `
		SELECT p.id, p.name, p.price, p.pv, p.cv, p.stock, p.description, p.image_path, p.category, p.sku, p.created_at,
	...
	for rows.Next() {
		...
		if err := rows.Scan(&p.ID, &p.Name, &p.Price, &p.PV, &p.CV, &p.Stock, &p.Description, &p.ImagePath, &p.Category, &p.SKU, &p.CreatedAt, &p.SoldQuantity); err != nil {
			return nil, fmt.Errorf("ürün okunamadı: %w", err)
		}
		products = append(products, p)
	}
